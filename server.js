...
MODE_PROMPTS['analysis'] = `## Mode: Analysis (Progressive Disclosure)
...
- **card.html**: Self-contained HTML with inline styles. You generate this from scratch each time using the Atomic Patterns below. The card body is max 560px wide.
...
MODE_PROMPTS['branching'] = `## Mode: Decision Branching (Comparison Columns)

You help users compare 2-3 concrete alternatives side by side. When the user asks a question that implies a decision (succession, restructuring, allocation), present options as comparison columns.

## Response Format

You MUST respond with a JSON object (no markdown fences, no other text):
{
  "message": "1-2 sentences. Brief context for the comparison.",
  "card": {
    "id": "unique-card-id",
    "title": "Short title (2-5 words)",
    "html": "<div>...parent card HTML with context...</div>"
  },
  "columns": [
    {
      "id": "col-unique-id",
      ...
      "riskLabel": "Low risk",
      "effects": [
        { "label": "Team continuity", "value": "High", "sentiment": "positive" }
      ],
      "people": [
        { "initials": "SC", "name": "Sarah Chen", "state": "normal|moved|added|risk" }
      ],
      "prompts": ["Beacon impact without Sarah?", "Readiness assessment"]
    }
  ],
  "decisions": []
}
...
### Field Rules
- **card**: Parent card providing context for the comparison. Always present.
- **columns**: 2-3 comparison columns. Each is a concrete alternative with effects, people, and drill prompts.
- **columns[].riskLevel**: "low", "medium", or "high" — determines tag color.
- **columns[].effects**: Key-value pairs showing tradeoffs. Sentiment: "positive", "negative", "neutral", "warning".
- **columns[].people**: People affected, shown as avatar dots. State indicates their role in this option.
- **columns[].prompts**: 1-3 drill prompts specific to this column. Clicking expands inline.
- **decisions**: Only populated when user clicks "Decide" on a column.

### Interaction Model
1. User asks a question → you present parent card + 2-3 comparison columns
2. User clicks a prompt inside a column → inline expansion shows the answer within that column
3. User clicks "Decide" on a column → that column glows green, others dim, decision logged
4. User can type in the ghost write-in column → you generate a new column from their idea

Use real graph data for all effects, people, and prompts. Never fabricate.`;

MODE_PROMPTS['allocation'] = `## Mode: Resource Allocation (Scenario Planning)

You help users plan team/resource reallocations by presenting scenarios with group buckets. The user can see how people are distributed across teams/projects and explore AI-suggested or custom scenarios.

## Response Format

You MUST respond with a JSON object (no markdown fences, no other text):
{
  "message": "1-2 sentences. Brief context.",
  "scenario": {
    ...
    "groups": [
      {
        ...
        "delta": -3,
        "people": [
          { "id": "person-001", "initials": "SC", "name": "Sarah Chen", "role": "Lead", "state": "normal|new|moved" }
        ]
      }
    ],
    "analysis": {
      "metrics": [
        { "label": "Headcount", "value": "0 net", "sentiment": "neutral", "note": "Internal redistribution" }
      ],
      "insights": [
        { "type": "pro|risk|con", "title": "Beacon gets unblocked", "description": "James Kim has the React expertise..." }
      ]
    }
  },
  "decisions": []
...
### Field Rules
- **scenario**: The allocation scenario to display. Contains groups (team buckets) and AI analysis.
- **scenario.groups**: Array of team/project buckets with their people assignments.
- **scenario.groups[].delta**: Change from current state (e.g. -3, +2). 0 or null if unchanged.
- **scenario.groups[].people[].state**: "normal" (unchanged), "new" (added to this group), "moved" (user-moved).
- **scenario.analysis.metrics**: 3-5 key metrics for the scenario.
- **scenario.analysis.insights**: 2-4 AI insights about the scenario. Type determines icon: pro (green check), risk (red !), con (yellow warning).
- **decisions**: Only populated when user clicks "Decide this scenario".

### Interaction Model
1. User asks about team allocation → you present current state + AI-suggested scenario
2. Scenarios appear as tabs; user can switch between them
3. User can drag people between groups to create custom scenarios
4. After edits, analysis goes stale; user clicks "Analyze changes" for fresh AI analysis
5. "Compare" button shows two scenarios side-by-side with key differences highlighted
6. User clicks "Decide this scenario" to commit

Use real graph data for all people, teams, and projects. Never fabricate.`;

function buildSystemPrompt(mode) {
  return SYSTEM_PROMPT_BASE + '\n\n' + (MODE_PROMPTS[mode] || MODE_PROMPTS['analysis']);
}
...
- **style**: "approve" (green), "warning" (amber), or "info" (blue). Choose based on urgency/impact.
