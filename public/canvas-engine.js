// canvas-engine.js — Spatial engine for EE Graph Studio
// Pan/zoom/blocks on an infinite canvas. Lifted from jit-ui-canvas, adapted for studio.
...
const CanvasEngine = (() => {
  ...
  // --- Blocks (standalone elements on the canvas) ---
  function addBlock(id, element, col, row) {
    element.classList.add('canvas-card');
  ...
  // --- getAllPositions: returns Map of id → {x, y, width, height} ---
  ...
  function zoomToFit(padding = 80) {
    const elements = world.querySelectorAll('.canvas-card, .canvas-section');
